const appid = "2986545e7e314c85b5314d2817f18cf4"
const { RTM } = AgoraRTM;
...
const initRtm = async (name) => {
  ...
  rtmClient = new RTM(appid,rtmUid)
  ...
  rtcClient.on("volume-indicator", volumes => {
    volumes.forEach((volume) => {
      ...
      try{
         ...
         if (volume.level >= 50){
           document.getElementsByClassName(`avatar-${volume.uid}`)[0].style.borderColor = '#00ff00'
         }else{
           document.getElementsByClassName(`avatar-${volume.uid}`)[0].style.borderColor = "#fff"
         }
      }catch(error){
        console.error(error)
      }
...
let handleMemberJoined = async (MemberId) => {
  ...
  try {
  let newMember = `
  <div class="speaker user-rtc-${userRtcUid}" id="${MemberId}" name="avatar-${volume.uid}">
